...
impl DmaChannel for tm4c_uart_dma_ctrl{
    ...
    fn dma_device_init(&mut self){
        ...
        self.device_dma.enaset.write(|w| unsafe{w.bits(0x100)});
         ...
         //Max transfer size 1024 is used.
 	     uart_rx_control_slice[2] = 0x0c00_0000 | 0x0CFFF1;

         //self.device_dma.cfg.write(|w| unsafe{w.bits(1)}); // enable when start function is invoked
    ...
    fn dma_stop(&mut self){
        self.device_dma.cfg.write(|w| unsafe{w.bits(0)});
    }
    ...
    fn dma_num_bytes_transferred(&mut self) -> usize{
        ...
        if((uart_rx_control_slice[2] & 0x7) == 0){
            ...
            1024 as usize - (outstanding_transfer_items + 1)
        }
    }
